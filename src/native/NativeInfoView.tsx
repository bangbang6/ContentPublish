...
type InfoViewGroupManagerProps = ViewProps;
const NativeInfoViewFrom =
  requireNativeComponent<NativeInfoViewFromType>('NativeInfoView');
const InfoViewGroupManager = requireNativeComponent<InfoViewGroupManagerProps>(
...
const NativeInfoView = () => {
  ...
  const sendCommand = (command: string, params: any) => {
    ...
    UIManager.dispatchViewManagerCommand(id, commands, params);
  };
  useEffect(() => {
    // 获取宽高
  ...
  return (
    <View>
      <View>
        <NativeInfoViewFrom
          style={styles.nativeInfoView}
          avatar={uri}
          ref={ref}
          name="mw2"
          desc="mw32323"
          onShapeChange={e => {
            console.log('e', e.nativeEvent.shape);
          }}></NativeInfoViewFrom>
      </View>
      <View>
        <InfoViewGroupManager style={styles.infoGroup}>
          <View
